...
def test_image_dir():
    ...
    with open(os.path.join(temp_dir, 'image1.jpg'), 'w') as f:
        f.write('dummy data')
    with open(os.path.join(temp_dir, 'subdir1', 'image2.png'), 'w') as f:
        f.write('dummy data')
    with open(os.path.join(temp_dir, 'subdir2', 'image3.gif'), 'w') as f:
        f.write('dummy data')
    with open(os.path.join(temp_dir, 'not_image.txt'), 'w') as f:
...
def test_get_directories(test_image_dir):
    ...
    # ベースパスを設定してテスト
    nested_dirs = get_directories(os.path.join(test_image_dir, 'subdir2'), 'subdir2')
...
def test_get_formatted_images(test_image_dir):
    ...
    # サブディレクトリの画像
    subdir_images = get_formatted_images(os.path.join(test_image_dir, 'subdir1'), 'subdir1')
